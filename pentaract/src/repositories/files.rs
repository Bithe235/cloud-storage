...
impl<'d> FilesRepository<'d> {
    ...
    pub async fn create_chunks_batch(&self, chunks: Vec<FileChunk>) -> PentaractResult<()> {
        ...
        .await
        .map_err(|e| {
            tracing::error!("database error in create_chunks_batch: {e}");
            PentaractError::Unknown
        })?;
    ...
    pub async fn set_as_uploaded(&self, id: Uuid) -> PentaractResult<()> {
        sqlx::query(format!("UPDATE {FILES_TABLE} SET is_uploaded = true WHERE id = $1").as_str())
            .bind(id)
            .execute(self.db)
            .await
            .map_err(|e| {
                tracing::error!("database error in set_as_uploaded: {e}");
                PentaractError::Unknown
            })
            .map(|_| ())
    }
    ...
    pub async fn list_dir(
        ...
        let fs_layer = fs_layer
            .into_iter()
            .map(|el| {
                let mut path = format!("{prefix}{}", el.name);
                if !el.is_file {
                    path.push('/');
                }
                FSElement {
