...
async fn main() {
    ...
    // running background cleanup worker
    let config_clean = config.clone();
    let db_clean = db.clone();
    tokio::spawn(async move {
        crate::services::cleanup_worker::start_background_cleanup_worker(
            db_clean,
            config_clean.telegram_api_base_url,
            config_clean.telegram_rate_limit,
        )
        .await;
    });
